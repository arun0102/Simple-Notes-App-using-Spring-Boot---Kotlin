...
import arun.pkg.sample_notes_spring_boot.security.AuthService
...
class AuthController(
    ...
    data class AuthRequest(
        ...
        @field:Pattern(
            regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{9,}$",
            message = "Password must be atleast 9 characters long and contain at least one digit, uppercase and lowercase character"
        )
        val password: String,
    )
